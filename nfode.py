...
def train(model, device, t_data, x_data, threshold=0.008, k=1, lr=0.01, milestones=[800], gamma=0.1):
    ...
    best_real_loss = 100
    while (best_real_loss >= threshold)&(epoch<=600):
        model.train()
        optimizer.zero_grad()
        ...
        if (epoch + 1) % 10 == 0:
            ...
            print(k, epoch + 1, current_time, model_device, 'best real loss:', best_real_loss)
    ...
    return best_model, best_real_loss, epoch, time_spent
